...
var url = cards[i].imgUrl ; 
var alt = cards[i].imgAlt ; 
var title = cards[i].header ;
var desc = cards[i].description ;  
 
var result = gallery(url , alt , title , desc) ; 
 
main_grid.appendChild(result) ; 
  
}
 
function gallery(url,alt,title,desc){
console.log(desc) ;
var grid_Item = document.createElement('div') ; 
grid_Item.classList.add('gridItem');
...
header1.classList.add('cardHeader');
header1.innerText = title ;  
var para = document.createElement('p') ;
para.classList.add('cardText'); 
para.innerText = desc ;  
card_content.appendChild(header1);
card_content.appendChild(para);
grid_Item.appendChild(Card_main) ; 
Card_main.appendChild(card_content) ; 
return grid_Item ;
}